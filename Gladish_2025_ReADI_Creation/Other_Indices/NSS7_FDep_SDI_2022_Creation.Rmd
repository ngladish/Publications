...
This script documents how we construct and harmonize the comparison indices used in the manuscript. Some indices are **reproduced from raw ACS data** using published methods, while others are **imported directly from their official releases** and minimally processed for linkage and alignment.

Specifically:

- We **reproduce**, as closely as the published documentation allows, the:
  - Social Deprivation Index (SDI),
  - Neighborhood Stress Score (NSS7), and
  - French Deprivation Index (FDep),

  using 5-year American Community Survey (ACS) data and the original authors’ variable definitions, transformations, and aggregation rules.

- We **import** the:
  - CDC/ATSDR Social Vulnerability Index (SVI), and
  - Neighborhood Atlas Area Deprivation Index (NA-ADI),

from their official sources and perform only the steps needed to clean identifiers, handle suppressed values, and align geographies with our analytic files.

The resulting index datasets (at the census block group, tract, and county levels as applicable) are then used in the main analysis script ([Analysis.Rmd](https://github.com/ngladish/Publications/main/Gladish_2025_ReADI_Creation/Analysis.Rmd)) to compare ReADI with existing deprivation measures in terms of agreement, correlation structure, and associations with mortality.
...
states <- setdiff(unique(fips_codes$state_code), c("60", "66", "69", "74", "78"))
...
The CDC/ATSDR Social Vulnerability Index (SVI) is a composite index designed to quantify how socially vulnerable communities are to external stresses on human health, including natural disasters, disease outbreaks, and other public health emergencies. It was developed by the CDC/ATSDR Geospatial Research, Analysis, & Services Program (GRASP) and has been widely used for emergency preparedness, resource allocation, and equity-focused planning.

SVI is calculated at the census tract and county levels for multiple years (e.g., 2000, 2010, 2014, 2016, 2018, 2020, 2022) using U.S. Census and ACS data. Scores are expressed as percentile ranks from 0 to 1, with higher values indicating greater social vulnerability relative to other tracts or counties in the comparison set.
...
The SVI incorporates 16 variables grouped into four conceptual themes:

1. **Socioeconomic status**
   - Poverty
   - Unemployment
   - Lower educational attainment
   - Income / economic constraints (or housing cost burden and related measures in more recent years)

2. **Household characteristics / disability**
   - Age structure (e.g., % aged ≥65, % aged ≤17)
   - Disability
   - Single-parent households
   - Limited English proficiency (in newer versions)

3. **Racial/ethnic minority status and language**
   - Racial/ethnic minority status
   - English proficiency (in earlier versions; later versions shift this variable to household characteristics)
...
Within each year, the CDC computes:
- **Raw estimates** for each variable from Census/ACS tables,
- **Percentages** (EP variables) where applicable,
- **Percentile ranks** for each variable within the national (or state) distribution (EPL),
- **Theme scores** (SPL) by summing percentile ranks within each theme, and
- **Overall SVI score** (`RPL_THEMES`), the percentile rank of the summed theme scores across all tracts or counties.

Thus, SVI is fundamentally a rank-based index: interpretation is always relative to other areas in the same year and geography.

## Data source and year used here

In this project, we use the **official CDC/ATSDR SVI release** rather than recomputing any of its components from scratch.

- We download the **county-level SVI data** for **2022** from the CDC/ATSDR SVI website.
- We use the **overall national percentile score** (`RPL_THEMES`) as our primary SVI measure.
- The original data include both tract- and county-level files; for consistency with other indices we work primarily from the county file and then propagate those scores to finer geographies as needed.

All SVI values in this project are taken directly from the official CDC/ATSDR files. We do not modify the SVI formulas, re-estimate the components, or alter the ranking procedure.

SVI data can be downloaded from their [website](https://www.atsdr.cdc.gov/place-health/php/svi/svi-data-documentation-download.html) at the census tract and county levels for years  [2022.08.05](https://www.atsdr.cdc.gov/place-health/media/pdfs/2024/08/SVI2022Documentation_08.05.22.pdf), and [2022](https://www.atsdr.cdc.gov/place-health/media/pdfs/2024/10/SVI2022Documentation.pdf). It is exported as a rank from 0 to 1 and the SVI can be exported based on nationwide ranking or statewide ranking as well. There are four themes and each have a set of previously defined variables required. Percentile ranking values range from 0 to 1, with higher values indicating greater vulnerability.

## Geographic linkage and harmonization

Our analytic units for deprivation comparisons are census block groups (CBGs), tracts, and counties. Because SVI is provided at the tract and county levels, we harmonize it to our geographies as follows:

- **County-level SVI**  
  We use the county-level SVI file directly. The key identifier is the concatenated state–county FIPS code (5 digits), which we align with our county GEOID field.

- **Tract-level SVI** 
  When tract-level SVI is required, we use the official tract-level SVI file and merge on the full 11-digit tract GEOID (state + county + tract). We do not aggregate or modify tract SVI values.

- **CBG-level SVI (for comparison only)**  
  Where SVI is needed at the CBG level (e.g., to compare indices across a common CBG-level file), we **do not create a new SVI**. Instead, we:
  - Attach the county-level `RPL_THEMES` score to every tract and CBG within that county using the shared county FIPS, and
  - Treat this as a county-level SVI value repeated across all lower-level units in that county.

This preserves the original SVI definition and avoids fabricating sub-tract SVI scores that CDC does not produce.

## Treatment of rankings and limitations over time

Several properties of SVI are important for interpretation:

- **Rank-based, not absolute**: SVI scores are percentile ranks within a given year and geography. A tract with SVI = 0.8 in 2018 is in the top 20% most vulnerable tracts *for 2018*; this is not directly comparable to SVI = 0.8 in 2022.
- **Changes in input variables and definitions**: Over time, some input variables and thresholds change (e.g., introduction of below-150% poverty, housing cost burden, health insurance, shifts of variables between themes). This means SVI is not a fixed construct across all years.
- **Boundary changes**: Census tract boundaries are redrawn over time, so tracts in earlier years (e.g., 2000, 2010) may not match tracts in later SVI releases, even when nominally “the same” tract ID appears.

Because of these issues, **we do not use SVI for direct longitudinal comparison** (e.g., “change in SVI from 2010 to 2022” at the tract level). In this project, SVI is used as a **cross-sectional comparator** for 2022 deprivation patterns:

- to benchmark how closely ReADI and other indices track a widely used federal vulnerability measure, and  
- to examine correlation and alignment between indices at a fixed point in time.

## Role of SVI in this project

In summary:

- SVI is treated as an **external, policy-relevant benchmark** rather than something we are re-estimating.
- We use the **official county-level 2022 SVI (`RPL_THEMES`)** and attach it to our tract and CBG analytic files via FIPS-based merges.
- All SVI values and rankings are **directly inherited from CDC/ATSDR**; any differences we observe between SVI and other indices (ReADI, NA-ADI, FDep, NSS7, etc.) reflect genuine differences between independently constructed indices, not differences in our implementation of SVI.

Readers who require more detail on the construction of SVI (including exact Census tables, formulas, and ranking procedures) should refer to the official CDC/ATSDR SVI documentation for the corresponding year.
...
```{r, eval = F}
SVI_2022_C <- read.csv("https://raw.githubusercontent.com/ngladish/Publications/main/Gladish_2025_ReADI_Creation/Other_Indices/SVI_2022_US_county.csv", colClasses = c(FIPS = "character"), stringsAsFactors = FALSE)
stopifnot(all(nchar(SVI_2022_C$FIPS) == 5), 
length(unique(SVI_2022_C$FIPS)) == nrow(SVI_2022_C))
SVI_2022_C$GEOID <- SVI_2022_C$FIPS
...
```{r, eval = F}
SVI_2022_CT <- read.csv("https://raw.githubusercontent.com/ngladish/Publications/main/Gladish_2025_ReADI_Creation/Other_Indices/SVI_2022_US.csv", colClasses = c(FIPS = "character"), stringsAsFactors = FALSE)
stopifnot(all(nchar(SVI_2022_CT$FIPS) == 11), 
length(unique(SVI_2022_CT$FIPS)) == nrow(SVI_2022_CT))
SVI_2022_CT$GEOID <- SVI_2022_CT$FIPS
...
save(SDI_2022_CBG, file = "SDI_2022_CBG.RData")
```
  </div>
</div>
...
save(SDI_2022_CT_RawVars, file = "SDI_2022_CT_RawVars.RData")
...
save(SDI_2022_CT, file = "SDI_2022_CT.RData")
```
  </div>
</div>
